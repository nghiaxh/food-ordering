...
import { Button, Carousel, Empty, Form, Input, Rate, message } from 'antd'
    ...
    <div className="pb-16">
      ...
      <section className="relative overflow-hidden bg-stone-950">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-24 md:py-32">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-5 text-4xl font-extrabold leading-tight text-balance text-white md:text-6xl">
              {HERO.titleA} <span className="text-amber-500">{HERO.titleHighlight}</span>{' '}
              {HERO.titleB}
            </h1>
              ...
              <div key={b.title}>
                <div className="relative h-60 overflow-hidden rounded-3xl md:h-80">
                  ...
                  <div className="absolute inset-y-0 left-0 flex flex-col justify-center px-8 md:px-14">
                    ...
                    <h3 className="mt-3 max-w-lg text-2xl font-bold text-balance text-white md:text-3xl">
                      {b.title}
                    </h3>
                    <p className="mt-2 max-w-md text-sm text-stone-200 md:text-base">{b.subtitle}</p>
                    <Link
                      to="/foods"
                      className="mt-4 flex w-fit items-center gap-1 text-sm font-semibold text-amber-400 hover:text-amber-300"
                    >
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {FEATURES.map((f) => (
              <div key={f.title} className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-stone-200/60">
                <UiImg src={f.image} alt={f.title} imgClass="h-36 w-full object-cover" />
                <div className="p-5">
            ...
            {loading ? (
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                {Array.from({ length: 4 }).map((_, i) => (
                  <PulseBlock key={i} className="h-44" />
                ))}
              </div>
            ) : (
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                {categories.map((c) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-x-0 bottom-0 flex items-end justify-between p-4">
                      ...
                      <span className="flex h-9 w-9 translate-y-1 items-center justify-center rounded-full bg-amber-500 text-stone-950 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
      ...
      <Reveal>
        <section className="mx-auto max-w-7xl px-4 py-16">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 lg:gap-0">
            {STEPS.map((s, idx) => (
              <div key={s.no} className="relative lg:px-2">
                <div className="h-full overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-stone-200/60">
                ...
                {idx < STEPS.length - 1 && (
                  <span
                    aria-hidden="true"
                    className="hidden lg:absolute lg:top-1/2 lg:-translate-y-1/2 lg:left-full lg:w-2/5 lg:translate-x-0 lg:border-t-2 lg:border-dashed lg:border-amber-200"
                  />
                )}
              </div>
            ))}
        ...
        <section className="bg-white py-16">
          <div className="mx-auto max-w-7xl px-4">
            ...
            <div className="mt-10 text-center">
              <Link to="/foods">
                <Button size="large" icon={<UiIcon name="arrow-right" />}>
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {TESTIMONIALS.map((t, idx) => (
              <Reveal key={t.name} delay={idx * 100}>
                <div className="h-full rounded-2xl bg-stone-800 p-6 ring-1 ring-stone-700">
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            <div className="space-y-4">
              {CONTACT_ITEMS.map((item) => (
                <div
                  key={item.label}
                  className="flex items-center gap-4 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-stone-200/60"
                >
      ...
      <section className="mx-auto max-w-7xl px-4 pt-16">
        <div className="relative overflow-hidden rounded-3xl bg-amber-600 px-8 py-12 text-center">
          ...
          <h3 className="relative text-3xl font-extrabold text-balance text-white">
